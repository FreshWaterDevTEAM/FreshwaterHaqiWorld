...
public final class HaqiAttackTask extends BukkitRunnable {
    ...
    private void fireForPlayer(Player player, HaqiTier tier, float loudness) {
        ...
        // Custom sound if resource pack loaded
    ...
    private void tickAmbientMobSounds() {
        ...
        ambientTimer = 0;
        for (Player player : Bukkit.getOnlinePlayers()) {
            for (Mob mob : player.getWorld().getNearbyEntitiesByType(Mob.class, player.getLocation(), 24.0)) {
                if (Math.random() >= 0.02) {
                    continue;
                }
                ...
                mob.getWorld().playSound(mob.getLocation(), sound, cat, 0.7F, pitch);
            }
        }
    }
